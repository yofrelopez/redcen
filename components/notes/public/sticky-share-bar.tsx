...
export function StickyShareBar({ title, url }: StickyShareBarProps) {
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            // Show if at top (first 100px) OR scrolling UP
    ...
    return (
        <div
            className={cn(
                "fixed bottom-6 left-1/2 -translate-x-1/2 z-50 md:hidden transition-all duration-300 ease-in-out",
                isVisible ? "translate-y-0 opacity-100" : "translate-y-[150%] opacity-0"
            )}
        >
            <div className="bg-white/70 backdrop-blur-xl border border-white/20 shadow-xl shadow-black/5 rounded-full px-5 py-2.5 flex items-center gap-4">
