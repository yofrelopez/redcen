...
// COPIED FROM lib/cloudinary-og.ts to avoid import issues
function generateCloudinaryOgUrl(
...
async function diagnose() {
    console.log("🔍 Diagnosing OG Image Generation...");
    ...
    try {
        ...
        const authorName = note.author?.abbreviation || note.author?.name || "Redacción Central";
        ...
        if (response.ok) {
            ...
            if (cldError) {
                console.error(`🚨 Cloudinary Error: ${cldError}`);
            } else {
                // If it's a 400/404 without headers, maybe we can read the body (text)
                const text = await response.text();
                // Careful if binary
                if (text.length < 500) console.error("   Body:", text);
            }
        }
